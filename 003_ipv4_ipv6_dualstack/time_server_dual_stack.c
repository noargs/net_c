/**
 * it is possible for te listening IPv6
 * to accept IPv4 connection with dual-stack
 * socket. not all operating system support 
 * dual-stack. some linux distro does.
 * if OS support dual-stack then highly 
 * recommended to implement it
 * 
 * only minor changes to time_online.c i.e. IPv4
 * addition will change program to dual stack.
 * after the call to socket() and before bind()
 * we must clear IPV6_V6ONLY flag on the
 * socket. this is done with setsockopt()
 * 
 */
...
int main() {
    ...
    /**
     * this will enable dual_stack (i.e. IPv4 and IPv6)
     * socket connection
     */
    int option = 0;
